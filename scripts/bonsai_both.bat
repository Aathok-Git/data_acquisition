@echo off
REM Bonsai launcher (Both Ephys + Miniscope visualizers)
...
set "SCRIPT=%~dp0bonsai_both.bonsai"

REM Run Bonsai in headless mode and enable both visualizers.
REM The following flags are placeholders — adjust to match your Bonsai CLI.
"%BONSAI_EXE%" --headless --script "%SCRIPT%" --visualizer EphysVisualizer --visualizer MiniscopeVisualizer
